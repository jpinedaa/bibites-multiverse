...
| Backpressure | A paced backlog is journal depth. When it reaches `inboundQueueMax`, or a configured pre-custody population gate is closed, the sidecar refuses **upstream** with Contract B `MIGRATION_NACK` / `OVERLOADED` (`contract-b-m4.md` §6.6, §6.8), which is a peer-local refusal and therefore proof that no custody moved. The sender continues after that live destination to the next untried deliverable slot on the same axis, and past the axis in `contract-b-m4.md` §34's order once the axis is exhausted (B50). The population load is the last reported living population plus journaled inbound arrivals plus successful spawns not yet reconciled by the next heartbeat; this reserves capacity across the heartbeat/ACK race. |
