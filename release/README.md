...
The build creates an add-on archive for each platform. Release `0.3.4` publishes one executable
...
buttons the join card carries a `Latest release v0.3.4` line, so a visitor can tell what the
...
`0.2.5` and `0.3.4` were built with, because the hand-unpacked copy was that same package unpacked
...
4. **Tag the commit the artifacts were built from**, and push the tag:
   `git tag v0.3.4 && git push origin v0.3.4`. The page's links point into the tag, so the
...
5. **Create the release** with `dist/RELEASE-PAGE.md` as its body. Attach both add-on archives,
   ...
   gh release create v0.3.4 \
       release/dist/bibites-multiverse-0.3.4-*.zip \
       release/dist/bibites-multiverse-0.3.4-*.exe \
       ...
       --title "Bibites Multiverse 0.3.4" \
