...
Release `0.2.5` was a Windows install fix: the installer and the uninstaller no longer refuse when
...
Release `0.2.6` keeps a world's identity when you install again over it: a data root that already
holds a world is adopted with its own name and relay instead of being refused, and an uninstall
that keeps the world data returns the same world to the same slot on the next install.
