...
| **D6** | **Species catalog is a module inside the sidecar, post-MVP (M7).** | It shares the sidecar's storage and (later) DHT; a standalone service created a circular dependency in the earlier draft. | Off the MVP critical path. **D11 does not overturn this:** `multiverse-archive` is one central recorder on the relay's host, not a per-peer library, and it depends on no sidecar internals. Whether the M7 catalog seeds from the archive or supersedes it is open — see the research table. **The call moves into M5, ratified 2026-08-10** (`m5_considerations.md`, decision 3): a public archive fills on somebody else's timetable and nothing may evict, so M5 produces the retention rule — keep everything and size the disk, keep the ledger and prune genomes to a horizon, or graduate to the catalog now — and the graduation question is answered with it rather than waiting for M7. **Answered in two steps, and the second one is a fourth option none of the three named.** On **2026-08-12** the owner took the middle one: keep the ledger, prune genome blobs to a horizon (`contract-b-m4.md` §23). On **2026-08-17**, against the measurement that the first answer was not funded to the end of its own announced run, the owner took **keep the record and window the lines** (§26): the archive writes its aggregates down and keeps them forever, and the per-crossing lines behind them are kept for the genome horizon on the host and off-host for the run. **The catalog therefore inherits a complete fold whose raw lines age out**, which is a different inheritance from the one §23 promised and a better-defined one — the fold is a normative list (§26, B39) rather than whatever a replay happened to compute. **Third step, 2026-08-25** (`contract-b-m4.md` §32): the genome blobs and `metrics.jsonl` gain the same cold-copy discipline the ledger has. A genome that ages out of a hot working set is bundled, copied off-host and restored on demand rather than deleted at the horizon, so **the raw lines and the cold blobs are archived off-host, not lost** — the catalog inherits a fold plus a reachable cold archive of the blobs behind it, not a fold beside a hole where old genomes were. |
...
| **D11** | **Lineage annex on every envelope, recorded by a new `multiverse-archive` service.** The annex carries the parent entity IDs plus a content hash of each parent genome. The mod supplies the parents as opaque blobs and the **sidecar** hashes them, because D4 keeps the body opaque to the mod. The archive runs beside the relay, records every envelope and annex, and fetches an unknown genome by hash from the source sidecar. | Migration is already the moment a genome crosses a machine boundary, so it is the cheapest possible place to build the organism database — and that database is the seed of the M7 species catalog. Without the annex an archive holds a pile of unrelated arrivals; with it, the same stream is a lineage graph. The archive sits **outside** the relay because D1 makes the relay dumb, and a relay that indexes genomes is no longer a frame forwarder. | New `multiverse-archive` component. Contract C gains the annex; Contract B gains a genome fetch by hash. **Known limit:** a database built from migrations holds migrants and their ancestors only, never the resident population. Periodic census uploads are a later option, not M3. **Amended 2026-08-12 and again 2026-08-17 on what the archive KEEPS, not on what it records.** §23 gave the genome store a retention horizon and left the ledger whole; §26 splits the ledger itself — **the aggregates the lines fold into are written down and kept forever, and the per-crossing lines are kept for a window equal to that horizon, compressed once closed, and removed only after an off-host copy is confirmed**. Every envelope and annex is still recorded, and the known limit above is unchanged and stays exactly as true: windowing the lines does not add the resident population to a database built from migrations. What it does change is that *"which individual crossed when"* is a question with a horizon on it, while *"which species crossed, how often, and descended from what"* is not. **Amended again 2026-08-25** (§32): the genome blobs gain a hot window and a receipt-gated cold tier, and `metrics.jsonl` gains a window folded into durable history. *"Which individual crossed when"* is answerable from the off-host copy for the run, and the individual's genome stays fetchable from cold storage rather than being deleted at the horizon — the record is made more durable, and the known limit above is unchanged. |
...
| **D19** | **Live hop animation.** When a receiving game acknowledges a migration spawn, the live map animates the **species glyph** — the coloured bibite silhouette the census view already draws — travelling along that lane's arrow in near-real time. The archive gains a **recent-hops feed**: lane, species names and timestamp for roughly the last 60 seconds, bounded by both time and count, served beside `/api/status` and polled on the page's existing 2-second cycle. The client animates the glyph between polls, arrival-triggered and brighter than the ambient pulses. **Corrected 2026-08-23:** the archive correlates copied `MIGRATION_PAYLOAD` offers with copied `MIGRATION_ACK` delivery proof. Rejected offers do not animate, and a reroute resolves to its eventual acknowledged destination. | The map already pulses per lane at a rate derived from `recentHops`, but **a pulse is anonymous** — it says traffic is flowing and nothing about what is flowing. The census gave every species a colour and a shape, and the ledger already carries a species block on every `MIGRATION_PAYLOAD`, so both halves of "which species is crossing which lane, right now" were already inside the archive and only the join was missing. Population admission exposed the missing distinction: seeing an offer is not seeing it land. The delivery ACK is the second input that makes the animation honest. It is also the cheapest possible verification of the two decisions above: two-way lanes show as glyphs travelling **both ways along one arrow**, and the exclusion shows as a species that is everywhere in the census and never on a lane. | Archive-internal and page-only. **No wire change**, and `contract-b-m4.md` §10.1's rules are the ones it already has to obey rather than new ones: the feed is built from envelope copies the archive already records, so "one source, no polling" is untouched; a hop whose payload carried no species block animates the **neutral glyph**, never a guessed name; and the feed is **delivery history — what a receiver acknowledged — never abundance** (D11, B12), so it may not be summed into a population. The bound is not optional: `Status` is serialized verbatim into `metrics.jsonl` once a minute, so an unbounded array on that struct would land in a durable file every minute forever. |
...
- Routing: the export edge → the ring's next slot east (D8). From M4 that target is the
  ...
  **An exact full-queue proof continues after the refused destination.** The relay session,
  ...
  the next compatible, untried slot on the same axis, then the rest of the map in a stated
  order once the axis is exhausted (B50). Exhaustion of the map, `maxReroutes`, or the one
...
- **Deferred pacing for relay sends.** The sidecar derives one send budget from the frame and byte
  limits in `HANDSHAKE_ACK`. Deferred frames wait for this ACK because the sidecar does not
  know the session limits before it. `MIGRATION_PAYLOAD` and ordinary journal
  `MIGRATION_ACK` frames share the deferred budget. A deferred ACK keeps `AckedUpstream`
...
- **Immediate response reserve.** `PONG`, other liveness frames, immediate `MIGRATION_NACK`,
  ...
  until the budget recovers. A tombstone re-ACK does not clear `AckedUpstream`. The deferred
  journal send clears it after transport acceptance.
...
- **Bounded transport queues for migrations.** Each destination connection has a separate queue
  for byte-identical `MIGRATION_PAYLOAD` frames. `F` is `maxFramesPerSecond`. `B` is
  `maxBytesPerSecond`. The queue retains at most `F` frames and `B` frame bytes.
  It stores opaque transport frames and counters, not decoded bodies or migration jobs.
  It is connection transport buffering, not store-and-forward routing or custody.
...
- **Identity-owned writer pacing.** The writer sends queued migrations at
  ...
  destination identity. A second connection cannot double the write rate.
- **Bounded control priority.** While a migration turn is not due, non-migration frames can
  move ahead freely. After a turn becomes due, at most `MigrationControlBurst = 7`
  non-migration frames can move ahead before the migration writes. This bound prevents
  migration starvation. The formulas define `R` and the burst. Neither value is an operator
  knob.
- **A conservative accepted-frame lifetime.** A relay-accepted migration stays with its selected
  destination connection. Any connection-local close, including replacement, drops that
  connection's queued migrations. At most one migration that the writer already selected can
  finish. The relay does not transfer the backlog, re-route it, or send a later NACK. The
...
archive **was** the one component here that **grew without bound on purpose**. Its genome
store gained a horizon with Decision 3 (`contract-b-m4.md` §23) and its ledger gained a
window on 2026-08-17 (§26). **What is unbounded now is what it KNOWS rather than what it
HOLDS**: the aggregates — species counts and first and last sighting, ancestry edges and
the ancestry floor, per-lane and per-peer totals, the coverage denominator, the record
counter — are kept forever and are bounded by construction, because their size follows the
number of species, lanes, peers and buckets and not the number of crossings. The
per-crossing lines behind them are kept for the genome horizon on the host, compressed
once closed, and off-host for the run. The figures below are the measurement that forced
the change and are kept as the reason for it, not as the current shape.
Measured on the six-slot deployment on 2026-08-08: **58 MB/h** of genomes,
...
**three months** on the 251 GB volume. Somebody had to decide what the archive is for before
that ran out; every other file in this system is bounded (see `dev_environment.md`,
*The disk budget*, which holds the full table and is the number of record). **They decided
twice — 2026-08-12 for the blobs and 2026-08-17 for the lines — and the second time the
number was worse**: on the public deployment the ledger alone runs `1.0`–`1.3 GB` a day,
which is more than the service host's volume holds for the rest of the announced run.
...
8. **From M4 (D12), a hop whose destination went dark is re-routed, bounced or lost —
   ...
   `forwardTimeoutMs` — 5 minutes by default since 2026-08-19 (`contract-b-m4.md` §27 B42;
   24 hours before that) — the entry is recorded **lost** and the
