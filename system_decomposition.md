...
2. **A hold is bounded** (D2, D12). An orphaned in-flight organism waits a configurable
   ...
   residual duplication risk. *Overrides the recommendation.*
...
question that document declined to answer for him. The remaining four
...
- Migration journal: durable custody of in-flight organisms, recovery on restart (D2).
  ...
  (D12), and its **hold deadline**, because a forwarded-then-silent entry bounces home
  when the timeout expires (D2, as amended). **From 2026-08-08 the journal is also bounded
...
- Compatibility enforcement at connect. **D22 makes this a contract-version gate**: the relay
  refuses a sidecar whose contract version it cannot speak and publishes the minimum it
  accepts. It does **not** police game versions — those are per-machine (the support matrix) —
  and the gate is a compatibility control, never a security one, because a claimed version is
  attacker-chosen text (`contract-b-m4.md` §13 item 7)
...
8. **From M4 (D12), a hop whose destination went dark is re-routed, bounced or held —
   ...
   **The hold is bounded** (owner sign-off, 2026-08-05): after a configurable timeout,
   24 hours by default, the entry bounces home on its own. `--release-inflight` ends a
   hold earlier when an operator has better information than the timer.
...
- **A map that heals and grows** (D12). A dark slot is routed around, not waited on; a
  ...
  A hold now expires: after a configurable timeout, 24 hours by default, the organism
  bounces home.
