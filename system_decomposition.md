...
question that document declined to answer for him. **D22 was then refined on 2026-08-11**, by
the same hand: the payload question it named as its own remaining work is settled by
*assumption plus a diagnostic field* rather than by research, which takes the `bb8-schema`
cross-version row back off M5's critical path. Its row carries the call. The remaining four
