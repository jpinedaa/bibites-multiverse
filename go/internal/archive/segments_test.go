...
func newTestArchive(t *testing.T, dir string, window time.Duration) *Archive {
	...
	return &Archive{
		...
		log:    testLogger(),
		ledger: l,
		// The aggregates the status view reads. A hand-built archive that skips
		// them renders a page nobody could serve, and StatusView is what this
		// file asserts on.
...
// copyFile is rollup_test.go's — one helper, one definition. The two branches
// each grew their own at the merge and phase 1's is the superset (it creates the
