...
Somebody downloads the release archive, runs `Install-BibitesMultiverse.ps1`, answers the join
string, and changes nothing else. That is the whole population this audit is about. Two facts
shape every verdict below:

1. **The package ships no relay and no archive.** The archive holds seven files — the installer,
   the uninstaller, the in-archive README, the mod, the sidecar, the BepInEx archive and the
   support matrix — so two of the four defaults are not reachable from a bare install at all.
   They are audited anyway, because the operator side is a hoster's bare install.
...
**A bare install.** Does not pass it. `Start-Multiverse.ps1` is generated by the installer and its
sidecar arguments are fixed: `--listen`, `--relay`, `--peer-id`, `--data-dir`, `--credential-file`.
A player would have to edit the generated script to add it.
...
**A bare install.** Keeps the starter species home. The installer writes
`MULTIVERSE_MIGRATION_EXCLUDE = 'Basic bibite'` explicitly into `Start-Multiverse.ps1`.

**What changed in the packaging, and it is a real fix rather than a note:**
