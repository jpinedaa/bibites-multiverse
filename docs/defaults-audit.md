...
strangers. This audit was updated for release `0.2.3` on **2026-08-15**, against the code and
...
identical in both: `--listen`, `--relay`, `--peer-id`, `--data-dir`, `--credential-file`. A player
would have to edit the generated script to add it.
...
`MULTIVERSE_MIGRATION_EXCLUDE` = `Basic bibite` explicitly into the start script they generate.
