...
| `INS-LAUNCHER-OPEN` **(Windows only)** | The GUI says installation succeeded and the connected game started, but the launcher could not open. It shows the launcher's path | The install and world start completed. Windows could not open the graphical launcher process | Open **Bibites Multiverse** from the desktop or Start Menu. You can also open the path that setup shows. If the file is absent, run the same setup again. Report the problem if it repeats | **you** |
| `INS-BUSY` **(Windows only)** | **Step 0** stops with *"something of this installation is running, and Windows does not let anything replace a program while it runs."*, lists each program by name, process id and path, and exits with code **3** rather than the usual `1`. The same refusal one step later, worded *"…is held open, so Windows will not replace it"*, means something started **during** the install | Windows holds a program's own file open for as long as it runs, and the launcher, the console launcher, the sidecar and the mod are all files an install replaces. **This is what a re-install over a live world used to hit at step 9**: a raw `Copy-Item : The process cannot access the file … because it is being used by another process` naming a line number in the installer, after steps 1 to 8 had already replaced the mod in the game and settled this world's identity — and with the setup around it quitting before it wrote the shortcuts or the *Installed apps* entry | **Stop every world first**: choose *"Stop every world"* in the launcher's window, or run `multiverse-launcher.exe stop --all` — `BibitesMultiverseLauncher.exe stop --all` in an install whose launcher has no separate console program — or `.\Stop-Multiverse.ps1` for a world this install's own script started. **Every one of those is lossless**; ending a world in Task Manager is not. Then close the **Bibites Multiverse** window itself, and The Bibites if it is still open, and run setup again. **NOTHING WAS CHANGED**, when it is step 0's refusal you are reading: step 0 asks before setup has even checked its own package. **The refusal one step later says otherwise itself**, because by then the mod and this world's identity are in place — and that is why the graphical setup's dialog no longer claims either way and points at the installer's own words underneath instead. Both refusals reach step 5 and step 9 alike: the mod is copied through the same lock-aware copy the launcher is, so a world started mid-install meets this refusal rather than a raw `Copy-Item` error. **Nothing is ever ended for you**, because a world ended rather than stopped loses everything it has simulated since its last save. The check is on this install's own folders — the application folder `-InstallRoot` names, and the game folder step 5 writes the mod into — so a second copy of the game, or another world's launcher elsewhere, does not block it; a process whose path this account cannot read decides nothing on its own, and what settles it then is whether that folder's own files are held open. **The Linux installer refuses the game half of this without an id**, for a reason the platform does not force on it: replacing a file the running game has mapped into memory can crash a world mid-save. The pair at the other end is `INS-UNINSTALL-BUSY` | **you** |
...
| `INS-UNINSTALL-CHANGED` | The uninstall's ledger says *"CHANGED since the install, so it is left alone"* against a path | A file the installer put there is no longer the file it put there. **In the complete edition's managed game copy it also keeps the folder**: that copy is reclaimed whole only when nothing the install recorded is left in it, so one file still vouching for it keeps everything around it too. **A ledger that says *"its data root is not a path this script will act on, so the whole profile stays"* against a complete edition's own profile is a bug that is fixed**: the rule that decides which paths a profile may name protected the managed game copy — which lives inside the data root — from the very run reclaiming it, so every profile was kept, `profiles\` stayed, and *Installed apps* could not remove the application folder around it. A game folder somebody CHOSE still keeps its data root out of reach, which is what that rule is for | Nothing, usually — this is the uninstall refusing to delete somebody else's work. Remove it by hand if you know what changed it. **On Windows the whole ledger is kept in `<data root>\logs\uninstall-<utc>.log`** (`%TEMP%` under `-RemoveWorldData`, and a `-DryRun` writes no file at all), so the line naming that path outlives the window it was printed in | **you** |
...
| `AOUT-JOURNAL_FULL` | transient | The sidecar already holds its configured maximum number of outbound migrations. This is an entry limit, not evidence that the disk is full | Wait. An acknowledgement, exact refusal, or forward timeout releases each entry by its custody rule. Do not delete or clear the journal. Use `--my-slot` to read the custody, paced, pending-ACK, and unresolved depths. If the depth stays at the limit while the game and relay remain connected, report it. Disk failures use `AOUT-JOURNAL_ERROR` and `LOCAL-DISK` instead | **nobody**, then **you** (report) |
...
| `LOCAL-DISK` | Writes fail; the sidecar reports journal errors; the machine fills | The genome cache grows to its cap, logs grow to their rotation ceiling, and the journal can expand between 15-minute compactions. A full disk here has previously torn an append-only record and left thousands of zero-byte scratch files behind, spending inodes when none remained | Free space and keep it free. Do not delete the journal: it is custody state and compacts to its live entries. On a packaged install, the sidecar writes under one data root — `%LOCALAPPDATA%\BibitesMultiverse` on Windows or `${XDG_DATA_HOME:-$HOME/.local/share}/bibites-multiverse` on Linux. `data/` holds the journal and genome cache, and `logs/` holds the rotated sidecar logs. Your worlds remain in the game's save directory. Keep space for all published ceilings and for growth between maintenance passes | **you** |
...
- **A forwarded migration that has no answer for up to five minutes.** The sidecar retries nothing.
  The sender handed over the organism once. It waits for an answer and takes no action without one.
  The sender does not send the organism again, and the organism does not come home. At
  `forwardTimeoutMs` — five minutes from the moment it was forwarded — the sidecar closes the entry
  as **lost**
