...
| `INS-MARKOFWEB` **(Windows only)** | Windows shows **Unknown publisher**, SmartScreen stops the setup, or an advanced ZIP script is blocked | The community setup is not code-signed. Windows also marked the downloaded file as internet content | First compare its SHA-256 with the release page. Then run `Unblock-File .\bibites-multiverse-0.2.3-windows-x64-setup.exe`, or select **Unblock** in its properties. If SmartScreen remains, select **More info → Run anyway** only for the matching file. Apply the same checksum-first rule to an advanced ZIP | **you** |
