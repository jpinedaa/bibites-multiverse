...
**The release always carries two add-on archives** and can carry an authorized complete archive
for either platform. One `SHA256SUMS` covers every published archive. Each archive has a
`MANIFEST.sha256`; a complete manifest also covers its game license, descriptor, and every game
file. The page's checksum covers the archive, and the archive's manifest covers its contents.
