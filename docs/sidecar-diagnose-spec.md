...
**Status: WP7's spine, with WP2's and WP4's texture landed.** Checks are stated against the wire
as WP1 published it, and the refusal wordings and published values the credential, TLS and
capacity work supplies are quoted from what ships. Where a value, a wording or an exit code
belongs to a package that has not landed — or to this package's own implementation arc — the
row carries a slot. §7 collects them.
