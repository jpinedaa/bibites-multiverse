...
| `journal-depths` | Are the custody, paced and unresolved depths healthy, and what has this world lost in transit? | Depths small and falling; nothing lost | It reports the four numbers in one line — **`in custody N, paced N, forwarded and unanswered N, lost N`** — and judges the paced one. `WARN` on a paced depth that never falls — **that names a delivery rate set too low**, and it is read against this world's own configured rate rather than against a default, which has been changed three times. **A single-shot tool cannot see a trend, so it reads an AGE instead of a slope**: at this world's own `inboundRatePerSimMinute` and its applied speed, the current depth needs a computable number of wall seconds to drain, and an entry that has already waited longer than the whole queue should take is a queue that is not draining. That is the configuration's own arithmetic and not a threshold. **An unanswered forward gets a line of detail and no verdict**, naming how long the oldest has waited and the fact a reader will otherwise get wrong: *it is NOT re-sent and does not come home; at `forwardTimeoutMs` it is recorded lost, because migration is at-most-once* (`contract-b-m4.md` §9.3). A non-zero `lost` is likewise a reading and not a fault — the map's price for never duplicating an organism (§25, B37) | `BMIG-OVERLOADED`, `AOUT-RATE_LIMITED` | The settled reading of the live map: all four depths at zero, no bypass; *Watch items* on reading a paced depth against its rate |
...
| **What it shows** | place and liveness; the map link and the last thing that refused it; this slot's `lastRefusal` and its last placement answer; the game, its versions, its population, and **applied speed beside achieved speed**; the last save against this world's own interval; one line per declared lane with its reason; the custody, paced and **unresolved** depths **with the delivery rate they are queued behind and the age of the oldest entry**, and beside them what this world has **lost** and what has answered **late**; and the whole map's worlds with their liveness, their game and their build |
| **What the custody block says, verbatim** | `unresolvedDepth`, `lostForwardTotal`, `lateAckTotal` and `oldestUnresolvedAgeMs` in the JSON — they replaced `heldDepth`, `bouncedTimeoutTotal` and `oldestHeldAgeMs` when the bounded hold was removed (`contract-b-m4.md` §25, B37) — and three rendered lines, each of which has to survive being read by a player: `unresolved   N forwarded once, waiting for an answer`; `lost         N forwarded and never answered. migration is at-most-once: an organism that does not arrive is gone, not returned`; and `late answers N arrived after their entry was written off; raise --forward-timeout`. **The last of those is good news and must not read as a fault**: a late answer means the organism arrived, exactly one exists, and the deadline is set shorter than this map's slowest honest answer. The two lost-and-late lines print only when their count is not zero, so an ordinary world's view is one line longer than before and no louder |
