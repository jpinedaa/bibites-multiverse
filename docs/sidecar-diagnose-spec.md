...
**Status: WP7's spine, with WP2's, WP4's and WP6's texture landed.** Checks are stated against the
wire as WP1 published it; the refusal wordings and published values the credential, TLS and
capacity work supplies are quoted from what ships; and the paths, log lines and matrix lookup the
package invents are quoted from the release. Where a value, a wording or an exit code belongs to
a package that has not landed — or to this package's own implementation arc — the row carries a
...
| §3 `limits` | The band that makes a reading *approaching* a limit a `WARN` rather than a `PASS`. The values themselves are no longer missing — every limit is a knob and the map publishes what it runs with — so the band is a fraction of a number read at runtime rather than a threshold any package can publish, and the implementation arc fixes it | **WP7**, later arc |
