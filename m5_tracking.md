...
**Status: nothing is started. Eight work packages, all *not started*, against a ratified plan.**
...
| **WP4** — Capacity, abuse limits, admin path | The published limit table, the authenticated admin path, the render deny list, and the three cheap contract closures | WP1, WP2 | **Not started** | Every limit is a knob, every peer-visible one is published on the stats block, and **every one is countable at the frame level** — D1 survives the package. Release, handover and eviction run over an authenticated path and keep the printed held-entry report. Contract Changes 11, 12 and 13 are closed |
