...
live at `bibitesmultiverse.com`, and its three-month announcement is integrated. WP6 is built,
but publication waits for a clean committed tree and a public repository. WP8 has not started.
...
| **WP6** — The package | A GitHub release a stranger can install from, with the defaults audit and the support matrix | WP2 (input: `farend/`) — **needs the game** | **Built 2026-08-12**, including Windows and Linux archives, native installers, uninstall proofs, checksums, the support matrix, and the defaults audit. WP3 supplied the real period text on 2026-08-15 UTC. Publication now waits for a clean committed tree, a public repository, the tag, and the GitHub release steps in `release/README.md` | An installer with no build toolchain and no instruction to disable a security control. Published checksums and security guidance precede the downloads. The support matrix ships beside the release and is enforced. Uninstall leaves the game as it found it |
